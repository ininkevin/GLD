    ...
    def forward(self, v, q, labels, bias, v_mask, q_mask, loss_type = None):
        ...
        if labels is not None:
            if loss_type == 'q':
                ...
                # loss = - (q_out.log_softmax(-1) * y_gradient).sum(1).mean(0)
                loss = - (q_out.log_softmax(-1) * labels).sum(1).mean(0)
                loss *= labels.size(1)
            elif loss_type == 'vq':
                ...
                loss = - (logits.log_softmax(-1) * y_gradient).sum(1).mean(0)
                loss *= labels.size(1)
        else:
            loss = None
...
def train(model, train_loader, eval_loader, qid2type, output):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (v, q, a, b, hintscore,type_mask,notype_mask,q_mask) in tqdm(enumerate(train_loader), ncols=100,
            ...
            pred, loss, _ = model(v, q, a, b, None, q_mask, loss_type = 'vq')
            if (loss != loss).any():
                raise ValueError("NaN loss")
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 0.25)
    ...
    plot_cruve(score_list, eval_score_list, epoch, fig_path)
    fig_path = os.path.join(output, 'loss.jpg')
    plot_cruve(loss_list, eval_loss_list, epoch, fig_path)

def plot_cruve(list, eval_list, epoch, fig_path):

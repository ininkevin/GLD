...
class BaseModel(nn.Module):
    ...
    def forward(self, v, q, labels, bias, v_mask, q_mask):
        ...
        if labels is not None:
            ...
            loss_q = - (q_out.log_softmax(-1) * y_gradient).sum(1).mean(0)
            ...
            loss = - (logits.log_softmax(-1) * y_gradient).sum(1).mean(0) + loss_q

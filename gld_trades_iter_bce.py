...
class BaseModel(nn.Module):
    ...
    def forward(self, v, q, labels, bias, v_mask, q_mask, loss_type = None, weight = 0.9):
        ...
        if labels is not None:
            if loss_type == 'q':
                loss = F.binary_cross_entropy_with_logits(q_out, labels)
                loss *= labels.size(1)
            elif loss_type == 'vq':
                ref_logits = torch.sigmoid(q_out) + bias 
                ref_logits = torch.clamp(ref_logits, min=0., max=1.) * labels
                loss_1 = F.binary_cross_entropy_with_logits(logits, labels)
                loss_2 = F.binary_cross_entropy_with_logits(logits, ref_logits)
